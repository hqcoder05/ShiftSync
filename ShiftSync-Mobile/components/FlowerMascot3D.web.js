...
/**
 * FlowerMascot3D.web.js — Bộ Đôi Mascot 3D Popmart Siêu Cưng Tương Tác
 * ─────────────────────────────────────────────────────────────────────────────
...
export default function FlowerMascot3D({
  ...
  useEffect(() => {
    ...
    camera.position.set(0, 0.1, 4.3); // Kéo camera lại gần để 2 nhân vật to tròn rõ nét hơn
    ...
    // 2. Lighting (Tươi sáng, mềm mại dạng Popmart vinyl toy bóng bẩy)
    ...
    // 3. Materials (Màu sắc vinyl popmart bóng mịn, nịnh mắt)
    const yellowMat = new THREE.MeshStandardMaterial({
      color: '#FFD426', // Vàng ấm tươi tắn rực rỡ
      roughness: 0.28,
      metalness: 0.05,
    });

    const creamFaceMat = new THREE.MeshStandardMaterial({
      color: '#FFFBEB', // Mặt kem vàng nhạt mịn màng bé bạch tuộc
      roughness: 0.35,
      metalness: 0.02,
    });

    const whitePorcelainMat = new THREE.MeshStandardMaterial({
      color: '#FFFFFF', // Mặt trắng sứ căng mịn bé vịt
      roughness: 0.22,
      metalness: 0.02,
    });

    const orangeBeakMat = new THREE.MeshStandardMaterial({
      color: '#FB923C', // Mỏ vịt cam mọng nước bóng bẩy
      roughness: 0.18,
      metalness: 0.1,
    });

    const orangeFeetMat = new THREE.MeshStandardMaterial({
      color: '#F97316', // Chân cam tròn xoe
      roughness: 0.3,
      metalness: 0.05,
    });
    ...
    const blushLavenderMat = new THREE.MeshStandardMaterial({
      color: '#F472B6', // Má hồng pastel bé vịt
      roughness: 0.45,
      metalness: 0.0,
    });

    const octopusMat = new THREE.MeshStandardMaterial({
      color: '#FB7185', // Bé bạch tuộc hồng san hô
      roughness: 0.28,
      metalness: 0.06,
    });
    ...
    octoMascot.position.set(-0.70, -0.05, 0); // Đứng gần nhau thân thiết hơn
    octoMascot.rotation.y = 0.28; // Quay sang bé vịt
    ...
    // ═════════════════════════════════════════════════════════════════════
    // NHÂN VẬT 2: BÉ VỊT VÀNG MỎ CAM LỌN TÓC XOĂN (Ảnh Mới - Bên Phải)
    // ═════════════════════════════════════════════════════════════════════
    const duckMascot = new THREE.Group();
    duckMascot.position.set(0.70, -0.05, 0); // Đứng gần bé bạch tuộc
    duckMascot.rotation.y = -0.28; // Quay sang bé tròn vàng
    duoRoot.add(duckMascot);

    // Mũ trùm đầu tròn to màu vàng (Big round duck hood)
    ...
    // Khuôn mặt trắng sứ tròn xoe mịn màng
    ...
    // LỌN TÓC XOĂN TÍT VÀNG TRÊN TRÁN (Đặc trưng siêu cưng trong ảnh mới)
    const curlGroup = new THREE.Group();
    curlGroup.position.set(0, 0.38, 0.74);
    // Vòng tròn xoắn ở giữa
    ...
    // Đôi mắt dẹt lim dim ngơ ngác đáng yêu (Ảnh mới)
    ...
    // MỎ VỊT CAM CĂNG MỌNG BÓNG BẨY (Thương hiệu bé vịt)
    ...
    // Thân áo choàng liền chuông màu vàng
    ...
    // 2 Bàn chân vịt cam tròn xoe bên dưới
    ...
    // ═════════════════════════════════════════════════════════════════════
    // 5. PHÁO GIẤY CONFETTI 3D & NGÔI SAO LẤP LÁNH (SPARKLES & CONFETTI)
    // ═════════════════════════════════════════════════════════════════════
    const fxGroup = new THREE.Group();
    duoRoot.add(fxGroup);

    // 24 Mảnh pháo giấy confetti ngũ sắc
    ...
    // 7. Animation Loop (2 bé giỡn qua lại, nhún nhảy, pháo giấy bay)
    let animId = null;
    ...
    const animate = () => {
      ...
      // Damping xoay kéo thả
      if (!isDragging.current) {
        duoRoot.rotation.y += rotVelocity.current.y;
        rotVelocity.current.y *= 0.92;
        rotVelocity.current.x *= 0.92;
        duoRoot.rotation.y += 0.0025; // Tự động xoay nhẹ khoe toàn cảnh 3D
      }

      // Tốc độ nhún nhảy khi hover nhanh và hào hứng hơn
      const freq = isHovered ? 6.8 : 3.4;
      const bounceAmp = isHovered ? 0.14 : 0.055;

      // ── BÉ TRÒN VÀNG: Nhún nhảy & lắc lư lạch bạch ──
      ...
      // ── BÉ VỊT VÀNG: Nhún so le trêu bé bạch tuộc ──
      ...
      curlGroup.rotation.z = Math.sin(elapsed * 4.5) * 0.12; // Lọn tóc xoăn tít đung đưa

      // Hai bé ghé nhìn nhau hoặc nhìn người dùng
